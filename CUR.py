...
def spearman_corr(matrix,ultimate):
    """
    Helper function to find Spearman Rank Correlation
...
def precision_on_K(matrix, ultimate):
    """
    Helper function to calculation Precision on Top K
    ...
    -------
    (float) precision
        Precision on top k value is returned
...
def CUR_decomposition(k):
    ...
    ----------
    k : int
        the reduced dimensionality after decomposition
...
def CUR_decomposition_90(k):
    """
     Performs the CUR decomposition on the input matrix with 90% retained energy
